...
def _progress_bar(current: int, needed: int, length: int = 12) -> str:
    filled = max(0, min(length, round(length * current / needed))) if needed else 0
...
class Leveling(commands.Cog):
    ...
    def _build_leaderboard_embed(self, guild: discord.Guild, guild_data: dict) -> discord.Embed:
        ...
        embed = discord.Embed(title="🏆 Classement du serveur", color=discord.Color(COLORS["gold"]))
        ...
        if not ranking:
            ...
            for i, (user_id, entry) in enumerate(ranking):
                ...
                bar = _progress_bar(entry["xp"], needed)
                hours = entry.get("voice_seconds", 0) / 3600
                lines.append(
                    f"{rank_icon} **{name}** — Niveau **{entry['level']}**\n"
                    f"`{bar}` {entry['xp']}/{needed} XP · 🎙️ {hours:.1f}h vocal"
                )
            embed.description = "\n\n".join(lines)
    ...
    async def niveau(self, interaction: discord.Interaction, membre: discord.Member = None):
        ...
        embed.add_field(name="Progression", value=f"`{_progress_bar(entry['xp'], needed)}` {entry['xp']}/{needed} XP", inline=False)
